...
class TestCodeGraphAgent:
    ...
    @patch('src.agent.tool_registry')
    @patch('src.agent.llm_client')
    async def test_process_query_no_tools_selected(self, mock_llm_client, mock_tool_registry):
    ...
    @patch('src.agent.tool_registry')
    @patch('src.agent.llm_client')
    async def test_process_query_tool_execution_error(self, mock_llm_client, mock_tool_registry):
        ...
        assert "response" in result
        assert "error" in result["response"].lower() or "Error occurred" in result["response"]
    ...
    async def test_generate_response_no_llm_client(self, mock_llm_client):
        ...
        mock_llm_client.generate_intelligent_response.side_effect = Exception("LLM not available")
    ...
    @patch('src.agent.tool_registry')
    @patch('src.agent.llm_client')
    async def test_agent_with_keyword_fallback(self, mock_llm_client, mock_tool_registry):
        """Test agent behavior when LLM falls back to keyword selection."""
        mock_tool_registry.list_tools.return_value = [
            {"name": "security_tool", "description": "Security analysis", "category": "Security"},
            {"name": "quality_tool", "description": "Quality analysis", "category": "Quality"}
        ]
    ...
    async def test_agent_with_empty_query(self, mock_llm_client, mock_tool_registry):
        ...
        assert "response" in result
        assert "error" in result["response"].lower() or "empty" in result["response"].lower()
